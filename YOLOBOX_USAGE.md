...
|---|---|---|
| **Hostname / IP address** | The address of the Linux box | `192.0.2.10` |
| **Port** | The SSH port (not always the default 22) | `22222` |
| **Username** | Your user account on the Linux box | `youruser` |
...
```powershell
ssh -p 22222 youruser@192.0.2.10
```

The default user account is `youruser`. The `-p 22222` flag specifies the non-standard port.
...
```powershell
ssh -p 22222 -i C:\Users\<you>\.ssh\my_key youruser@192.0.2.10
```

**First connection:** You will see a fingerprint prompt like this:

```
The authenticity of host '[192.0.2.10]:22222' can't be established.
...
Add `-v` for verbose output to help debug: `ssh -v -p 22222 youruser@192.0.2.10`
...
```
[PowerShell] ssh -p 22222 youruser@192.0.2.10
